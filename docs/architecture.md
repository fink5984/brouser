...
- **Website HTTP Basic-Auth isn't auto-handled.** `onReceivedHttpAuthRequest` fires for both the proxy's own 407 challenge and an origin server's 401, and androidx.webkit doesn't distinguish the two in the callback. Since every request in this app goes through the proxy, the handler always supplies the proxy's credentials rather than trying to tell the two apart -- worst case for a genuine destination-site Basic-Auth prompt (rare in 2026), the wrong credentials are tried and it fails the same way cancelling would have. A real login dialog for that case is a contained addition to `BrowserWebView.InnerWebViewClient` if a site ever needs it.
